 ...
 */
int sm2tc(int x) {
   int m=(~((~0)<<31))&x; // create a speacial number
   int sign=x>>31;  // get sign bit
   int plus=(x>>31)&1;// and sign bit with 1 to make sure we only add 1 when 
                      // sign is 1, plus =0 otherwise 
   int k=(sign^m)+plus; // the special number xor m means ~m if x is negative 
...
int rotateRight(int x, int n) {
  int m=x<<(32+((~n)+1)); // get the section that needs to be put at front
  int p=~((~0)<<(32+((~n)+1))); // get the special number p
  int k=x>>n;
...
int divpwr2(int x, int n) {
   int sign=x>>31; // get the sign of the x
   int bitdiv=1<<n;// get the divider of the function 
...
int allOddBits(int x) {
  ...
  int k=m&x; // and the number m with x and we will get m again if x has all 
             //0 odd bits 
...
int isTmin(int x) {
   int m=0; 
   int y=x+(~m); //use ~0 to represent -1
